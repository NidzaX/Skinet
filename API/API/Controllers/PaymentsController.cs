    ...
    IHubContext<NotificationHub> hubContext) : BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        try
        {
            ...
            await HandlePaymentEventSucceeded(intent);

            return Ok();
        }
        catch (StripeException ex)
        {
            logger.LogError(ex, "Stripe webhook error occurred");
            return StatusCode(StatusCodes.Status500InternalServerError, "Webhook error occurred");
        }
        catch (Exception ex)
        {
    ...
    private async Task HandlePaymentEventSucceeded(PaymentIntent intent)
    {
        if (intent.Status == "succeeded")
        {
            ...
            {
                logger.LogInformation("Payment already processed for Order ID: {OrderId}", order.Id);
                return;
            }
